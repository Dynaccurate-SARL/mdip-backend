...
def test_dk_open_and_validate_invalid_file():
    # Arrange
...
def test_dk_parse_valid_data():
    # Arrange
    mock_file = io.BytesIO()
    valid_data = pd.DataFrame([
        {"Drugid": "A2", "Navn": "Attack 2", "Extra": "android"},
        {"Drugid": "2B", "Navn": "2 Mo. Battle", "Extra": "android"},
    ])
    valid_data.to_excel(mock_file, index=False, engine="openpyxl")
    mock_file.seek(0)

    # Act
    parser = DK_Parser(mock_file)
    parser.parse()
