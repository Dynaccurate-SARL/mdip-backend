...
def test_pt_open_and_validate_invalid_file():
    # Arrange
...
def test_pt_parse_valid_data():
    # Arrange
    mock_file = io.BytesIO()
    valid_data = pd.DataFrame([
        ["Nome do Medicamento", "Extra"],
        ["Attack 2", "android"],
        ["2 Mo. Battle", "android"]
    ])
    ...
    # Act
    parser = PT_Parser(mock_file)
    parser.parse()

    # Assert
    assert sorted(parser._df.columns) == ["drug_code", "drug_name", "properties"]
    assert parser._df.iloc[0]["drug_code"] == "PT_1"
