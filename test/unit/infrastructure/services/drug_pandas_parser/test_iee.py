...
def test_ee_open_and_validate_invalid_file():
    # Arrange
...
def test_ee_parse_valid_data():
    # Arrange
    mock_file = io.BytesIO()
    valid_data = pd.DataFrame([
        ["Name of medicinal product", "Extra"],
        ["Attack 2", "android"],
        ["2 Mo. Battle", "android"]
    ])
    ...
    # Act
    parser = EE_Parser(mock_file)
    parser.parse()

    # Assert
    assert sorted(parser._df.columns) == [
        "drug_code", "drug_name", "properties"]
    assert parser._df.iloc[0]["drug_code"] == "EE_1"
