...
class DrugMapping(Base):
    ...
    _drug_id: Mapped[int] = mapped_column(
        "drug_id", sq.BigInteger, sq.ForeignKey("drugs.id"), nullable=False
    )
    _related_drug_id: Mapped[int] = mapped_column(
        "related_drug_id", sq.BigInteger, sq.ForeignKey("drugs.id"), nullable=False
    )

    __table_args__ = (
        sq.PrimaryKeyConstraint(
            "drug_id", "related_drug_id", name="uq_drug_related_drug"
        ),
    )
