...
class GetAdminStatsUseCase:
    ...
    async def execute(self, kind: StatsKind) -> AdminStatsDto:
        ...
        if kind == "central":
            return AdminStatsDto(central_catalog=await self._get_central_catalog())

        # catalogs count only
        if kind == "total_catalogs":
            return AdminStatsDto(total_catalogs=await self._get_catalogs_count())

        # mappins count only
        if kind == "total_mappins":
            return AdminStatsDto(total_mappins=await self._get_mappings_count())
