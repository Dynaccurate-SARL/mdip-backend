...
class VerifyTransactionUseCase:
    def __init__(self, t_repository: TransactionRepositoryInterface,
                 ledger_service: LedgerInterface):
        self._t_repository = t_repository
        self._ledger_service = ledger_service

    async def execute(
            self, transaction_id: UUID) -> TransactionDto:
        payload = await self._t_repository.\
            get_payload_by_transaction_id(transaction_id)
        if payload is None:
